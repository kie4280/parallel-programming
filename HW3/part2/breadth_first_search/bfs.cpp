...
void bfs_bottom_up(Graph graph, solution *sol) {
  ...
  while (frontier->count != 0) {
    ...
    bottom_up_step(graph, frontier, new_frontier, f_num++, sol->distances);
...
void hybrid_step(Graph g, vertex_set *frontier, vertex_set *new_frontier,
                 int *distances, int *mf, int *nf, int *mu) {}
...
void bfs_hybrid(Graph graph, solution *sol) {
  ...
  // setup frontier with the root node
  frontier->count = 1;
  frontier->vertices[0] = FRONTIER_MARKER;
  sol->distances[ROOT_NODE_ID] = 0;
  ...
  while (frontier->count != 0) {
    ...
    new_frontier->count = 0;
    hybrid_step(graph, frontier, new_frontier, sol->distances, &mf, &nf, &mu);
